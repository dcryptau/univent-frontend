...
const CampusDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const campusResponse = await fetch(`${import.meta.env.VITE_APP_API_URL}/user/${id}`, {
                    headers: {
                ...
                // Fetch events
                const eventsResponse = await fetch(`${import.meta.env.VITE_APP_API_URL}/events/created`, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
                    },
    ...
    const handleDeleteEvent = async (eventId) => {
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/events/${eventId}/delete`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
