...
function Events() {
        ...
        const fetchEvents = async () => {
            try {
                ...
                const fetchSection = async (section) => {
                    const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/events?section=${section}`, { 
                        method: 'GET', 
                        headers 
                    });
    ...
    const handleEventSearch = async (query) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/events?search=${query}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`,
